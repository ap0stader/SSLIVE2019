...
$websocket->set(
    array(
        ...
        'ssl_cert_file' => '/var/cert/backend/backend.pem',
        'ssl_key_file' => '/var/cert/backend/backend.key',
        // https://wiki.swoole.com/wiki/page/280.html
        'log_file' => '/var/log/html/swoole.log'
    )
);
...
$websocket->on('open', function ($websocket, $request) {
    $file = fopen("/var/log/html/php.log", "a");
...
$websocket->on('message', function ($websocket, $frame) {
    $file = fopen("/var/log/html/danmu.log", "a");
